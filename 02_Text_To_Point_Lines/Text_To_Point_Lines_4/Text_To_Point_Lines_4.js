// Text to point lines variation 04
// Change = Lines trail behind the mouse (help from p5.js tuts and VSC) 

let myFont;
let points;
let sampleF;
let mouseHistory =[];
let historyLength = 60; // how many past positions to relay
...
function draw() {
  ...
  background(100,0,30,25); // added opacity to background
  
  //CHANGE
  
  mouseHistory.push({ x: mouseX, y: mouseY }); // current mouse position
  if (mouseHistory.length > historyLength) { // Keep history capped at this Length
    mouseHistory.shift();
  }
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    let target = mouseHistory[floor(random(mouseHistory.length))]; 
    let recency = map(mouseHistory.indexOf(target), 0, mouseHistory.length, 20, 120);

  
    stroke((255),(255),(255), recency);
    strokeWeight(0.5);
...
function keyPressed() {
  if (key === 's') {
    saveGif('Text_To_Point_Lines_4', 5); // Saves 5 seconds of animation
  } 
}
